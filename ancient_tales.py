...
if "crew_index" not in st.session_state:
    st.session_state.crew_index = 0
if "diving_step" not in st.session_state:
    st.session_state.diving_step = 0
if "pearl_index" not in st.session_state:
    st.session_state.pearl_index = 0
if "ship_question_index" not in st.session_state:
    st.session_state.ship_question_index = 0

# -----------------------
# MENU + CHARACTER SELECTION ON IMAGE
# -----------------------
if st.session_state.page == "menu":
    bg = load_image("menu_background.png")
    if bg:
        text = "🏝️ Welcome to Ancient Tales\nChoose your character below:"
        bg_text = draw_text(bg, text, position=(50,50), font_size=50)
        st.image(bg_text, use_column_width=True)

    col1, col2 = st.columns(2)
    with col1:
        nayhan = load_image("nayhan.png")
        if nayhan:
            nayhan_img = draw_text(nayhan, "Nayhan", position=(50,180), font_size=40, color=(255,255,0))
            st.image(nayhan_img, width=300)
    ...
    with col2:
        ...
        if dhabia:
            dhabia_img = draw_text(dhabia, "Dhabia", position=(50,180), font_size=40, color=(255,255,0))
            st.image(dhabia_img, width=300)
...
# -----------------------
    ...
    if bg:
        bg_text = draw_text(bg, "🤖 Welcome to the Seaside Adventure!", position=(50,50), font_size=50)
        st.image(bg_text, use_column_width=True)
    ...
    if robot:
        st.image(robot, width=100)
    kids = load_image("kids.png")
    if kids:
        st.image(kids, width=200)
...
# -----------------------
    ...
    crew = [
        ...
        ("ghais.png", "Ghais - Main pearl diver."),
        ("seeb.png", "Seeb - Surface assistant."),
        ("naham.png", "Naham - Ship entertainer."),
        ("jallas.png", "Jallas - Pearl inspector."),
        ("skuni.png", "Skuni - Helmsman."),
        ("cook.png", "Tabakh - Responsible for food."),
        ("radif.png", "Radif - Assists Seeb.")
    ]
...
# -----------------------
# PEARL GAME (CLICK PEARLS, SHOW QUESTIONS, HEARTS, TIMER)
# -----------------------
